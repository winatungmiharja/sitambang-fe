...
type StokActionProps = {
  data: Stocks;
  mutate: KeyedMutator<ApiReturn<Stocks[]>>;
};

type EditData = Pick<Stocks, 'name' | 'priceStock' | 'totalStocks'>;

const StokAction = ({ data, mutate }: StokActionProps) => {
  ...
  const onDelete = () => {
    dialog({
      title: (
        <>
          Hapus Barang <span className='font-bold'>{data.name}</span>
        </>
      ),
      description:
        'Setelah Barang terhapus maka data tidak bisa dikembalikan, apakah Anda yakin?',
      submitText: 'Hapus',
      variant: 'danger',
    }).then(() => {
      toast.promise(
