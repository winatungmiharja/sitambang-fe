...
import { ApiReturn, Buyer, Payment, Stocks } from '@/types/api';
...
type EditData = Pick<Stocks, 'name' | 'totalStocks'> &
  Pick<
    Payment,
    'paymentMethod' | 'paymentImage' | 'purchasedStock' | 'idBuyer'
  > & {
    category: number;
  };
...
const AddTransactionAction = ({ data, mutate }: AddTransactionActionProps) => {
  ...
  const { data: apiBuyer } = useSWR<ApiReturn<Buyer[]>>('/buyer/view');
  const buyer: Buyer[] = apiBuyer?.data ?? [];
  //#endregion  //*======== Get Buyer Data ===========
                    ...
                    <div className='grid grid-cols-2 gap-8'>
                      <div className='flex flex-col gap-4'>
                        <SelectInput
                          id='idBuyer'
                          label='Pembeli'
                          placeholder='Harap Pilih Pembeli'
                          validation={{
