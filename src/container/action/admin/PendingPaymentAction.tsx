...
type PendingPaymentActionProps = {
  data: Payment;
...
const PendingPaymentAction = ({ data, mutate }: PendingPaymentActionProps) => {
  //!TODO
  ...
  const onDelete = () => {
    dialog({
      title: (
        <>
          Hapus Transaksi <span className='font-bold'>{data.totalPayment}</span>
        </>
      ),
      description:
      ...
      toast.promise(
        axiosClient.post('/transaction/update').then(() => {
          mutate();
        }),
        {
          ...DEFAULT_TOAST_MESSAGE,
                  ...
                  <form onSubmit={handleSubmit(onUpdate)} className='mt-8'>
                    <div className='grid gap-4 w-full'>
                      <Input
                        ...
                        id='name'
                        validation={{
                          required: 'Nama Transaksi baru harus diisi',
                        }}
                      />
